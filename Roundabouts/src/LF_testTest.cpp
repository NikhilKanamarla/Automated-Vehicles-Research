...
{
    // Nikhil adding comment for variable test
    int id = (int)req.id;
    ...
    {
        res.res = 1;
        ROS_INFO("Failed to give result. Error code: 1 (track unregistered) and id = [%lf]",id);
    }
...
int main(int argc, char **argv)
{
    ...
    ROS_INFO("map size value = [%lf]",theMap.trackList[64]); // try accesing this value
