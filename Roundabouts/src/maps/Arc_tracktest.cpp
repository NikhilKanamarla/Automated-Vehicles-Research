...
{
    double ang = atan2(y-_cY,x-_cX);//the range of atan2 is between -pi and pi 
    if(_angleS>_angleE){
        if(ang>_angleE||ang<(_angleS-2*PI)) return false;  //For example A81  s=324.74 and e=90->s=-35.26 , e=90
    }
    else{
        ang = ang<0?ang+2*PI:ang;
        if(ang>_angleE||ang<_angleS) return false;
    }
