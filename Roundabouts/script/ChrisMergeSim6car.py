...
#zumoThread inputs: index (car number) frameName (car name in Vicon), controlRate (???), path (car's path from path.py)
    ...
    road_speed = 0.3 #speed limit on the road unless intervened by from the controller
...
t1.start()
t1 = Thread(target = zumoThread, args = (index2, name2, 0.05, copy.deepcopy(Path.GetDefaultPath(8))))
t1.start()
