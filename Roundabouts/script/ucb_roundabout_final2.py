...
from rllib_control import RllibController
import random
...
TestActive = True #Uses Vicon Position as opposed to simulated Position
MultiAgent=True 
DEBUG = False
isRun = True
SAVE_DATA = True
...
stopDistance = .2 #Distance of car to start of stop sign (max)
carDistance = .4 #Distance between cars when stopped (max)
...
#b_min = 2
#b_max = 5
n_A = random.randint(a_min,a_max)
n_B = random.randint(b_min,b_max)
flush_a = True
flush_b = True
platoonAReady = False
platoonBReady = False
...
ind13=25
#
inds=[ind1,ind2,ind3,ind4,ind5,ind6,ind7,ind8,ind9,ind10,ind11,ind12,ind13]
#RouteA
...
def getRLAction(ind1, ind2, pathIndex1, pathIndex2):
    ...
    try:
        if not MultiAgent:
            ...
            a=control.get_action(np.asarray(obs))
...
def animate(i):
    ...
    ax1.scatter(stopB,(stopy_maxB+stopy_minB)/2,c='y',s=100)
...
def csvout():
    ...
    for i in range(nVehicles):
        ...
        vYL += 'viconY-'+str(i)+','
        vTL += 'viconT-'+str(i)+','
    header += pos + q1p + q1v + q2p + q2v + rbp + rbv + vXL + vYL + vTL+ 'flush,' + 'n_A,' + 'n_B' + '\n'
    #open a unique file for this data
    currentDate = time.strftime('%m_%d_%y__%H_%M_%S')
    datafile = open("/home/themainframe/catkin_ws/src/line_following/data/UCB/" + wtfile_name +  "_roundaboutRL%s.csv" %currentDate,"w")
    #write the header
	...
	if len(eBD) > 0:
        ...
        for i in range(nVehicles):
            ...
            vyD +=str(vy[i])+','
            vtD +=str(vt[i])+','
        data += pos + q1p + q1v + q2p + q2v + rbp + rbv + vxD + vyD + vtD + str(isflushing) + ',' + str(n_A) + ',' + str(n_B) +  '\n'
...
def write_stateobs():
    ...
    #open a unique file for this data
    currentDate = time.strftime('%m_%d_%y__%H_%M_%S')
    datafile = open("/home/themainframe/catkin_ws/src/line_following/data/UCB/" + wtfile_name +  "_state-obs%s.csv" %currentDate,"w")
    datafile.write("time[s]|observation|action\n")
    t0 = time.time()
    #grab and write data
    while isRun:
        data = str(time.time() - t0) + "|" + str(obs) + "|" + str(a) + '\n'
...
def zumoThread(index, frameName, controlRate, path,pathIndex):
    ...
    x = posx[index]
    y = posy[index]
    #Temporary Variables for checking velocity
    xold=x
    yold=y
    ...
    while isRun:
        ...
        #if index==7:
            #print("My goal speed is :" + str(speed),"My actual speed is " + str(speedReal))
        #    print('[' + str(speedReal) + ']')
        ...
        if status == 198: #if in queue (lower queue) (arc part)
           ...
           if not enteredExp and not (flush_a or flush_b):
               ...
               if localP>=randNum:
                  ...
                  eAD.append(index)
                  enteredExp = True
                  stopOn = True #enable the stop sign so we don't do 2 loops
           inExp=True
	   enteredExp=True
           if noInQueueA:
              inQueueA=inQueueA+1
	   ...
	   enteredExp=True
        ...
        elif ((n_A is not a_max) and index==indiciesA[n_A]) and inZone(stopA,stopA+stopDistance,stopy_minA,stopy_maxA,posx[index],posy[index]) and not flush_a:
            speed=0.0
        elif index == indiciesB[0] and inZone(stopB-stopDistance,stopB,stopy_minB,stopy_maxB,posx[index],posy[index]): #Stop signs
            if stopOn:
                speed = 0.0
                leadInStopB=True
                flush_b=False
                ...
                stopped = False
        elif ((n_B is not b_max) and index==indiciesB[n_B]) and inZone(stopB-stopDistance,stopB,stopy_minB,stopy_maxB,posx[index],posy[index]) and not flush_b:
            speed=0.0
        t7=time.time()
...
if MultiAgent:
    wtfile_name = "ma_state_noise"
    control=RllibController("/home/themainframe/catkin_ws/src/line_following/script/flow-transfer-master/data/rllib/"+wtfile_name,150)
else:
    wtfile_name = "varied_inflows_no_noise_2"
    control=RLController("/home/themainframe/catkin_ws/src/line_following/script/flow-transfer-master/data/weights/" + wtfile_name + ".pkl")
...
if TestActive:
    for i in range(12):
        x,y,t=getViconPos("zumoTest"+str(inds[i]))
        posx[inds[i]]=x
        posy[inds[i]]=y
#
time.sleep(1)
...
if (SAVE_DATA):
    ...
    twrite.start()
    tsobs = Thread(target = write_stateobs)
    tsobs.start()
