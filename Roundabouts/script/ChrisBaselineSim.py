#!/usr/bin/env python
# new file coming soon, I am just learning github for desktop
...
def animate(i):
    ...
    ax1.scatter(posx[21],posy[21],c='b',s=300)
    ax1.scatter(posx[22],posy[22],c='b',s=300)
    ax1.scatter(posx[12],posy[12],c='b',s=300)
    ax1.scatter(posx[15],posy[15],c='b',s=300)
    ax1.scatter(posx[5],posy[5],c='r',s=300)
    ax1.scatter(posx[10],posy[10],c='r',s=400)
    ax1.scatter(posx[33],posy[33],c='g',s=500)
    ax1.scatter(posx[31],posy[31],c='r',s=200)
    ax1.scatter(posx[32],posy[32],c='r',s=200)
    ax1.scatter(posx[26],posy[26],c='g',s=200)
...
t12.start()
t13 = Thread(target = zumoThread, args = (31, "zumoTest31", 0.05, copy.deepcopy(Path.GetDefaultPath(5))))
t13.start()
t14 = Thread(target = zumoThread, args = (32, "zumoTest32", 0.05, copy.deepcopy(Path.GetDefaultPath(6))))
t14.start()
t15 = Thread(target = zumoThread, args = (33, "zumoTest33", 0.05, copy.deepcopy(Path.GetDefaultPath(7))))
t15.start()
t6 = Thread(target = zumoThread, args = (12, "zumoTest12", 0.05, copy.deepcopy(Path.GetDefaultPath(0))))
t6.start()
t7 = Thread(target = zumoThread, args = (21, "zumoTest21", 0.05, copy.deepcopy(Path.GetDefaultPath(2))))
t7.start()
t8 = Thread(target = zumoThread, args = (15, "zumoTest15", 0.05, copy.deepcopy(Path.GetDefaultPath(3))))
t8.start()
t9 = Thread(target = zumoThread, args = (22, "zumoTes22", 0.05, copy.deepcopy(Path.GetDefaultPath(3))))
t9.start()
t10 = Thread(target = zumoThread, args = (26, "zumoTest26", 0.05, copy.deepcopy(Path.GetDefaultPath(4))))
t10.start()
