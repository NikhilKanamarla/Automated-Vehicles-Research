...
filename = "/home/themainframe/catkin_ws/src/line_following/data/data_10_17_17__21_38_42.csv"
...
for j in range(len(carIDs)): #using cur_vel
    ...
    for k in range(len(carDataMat[j])):
	    ...
	    #if loopCount==35: #insertlength
            #if float(carDataMat[j][k][0])>2.5 or float(carDataMat[j][k][0])<8.1:
		   #fit = np.polyfit(time,vel,2)
		   #fit_fn = np.poly1d(fit)
	           #plt.plot(time,fit_fn(time),color = "orange",linewidth = 3)
		   #new
	    #else: #if loopCount==116:
            fit = np.polyfit(time,vel,1)
            fit_fn = np.poly1d(fit)
            plt.plot(time,fit_fn(time),color = "orange",linewidth = 3)
