...
def dataCollection(data,index,path,x,y,cur_speed): ##
    timestamp = str(time.clock()) + ','
    data += timestamp + str(index)+ ',' + str(x)+','+str(y) + ',' + str(cur_speed)+ ';' #+','+str(battlev)
    return data

def mergeDataCollection(mergeData,index,x,y,cur_speed,to,tmi,abcd): ##
    timestamp = str(time.clock()) + ','
    mergeData += timestamp + str(index)+ ',' + str(x)+','+str(y)+',' + str(cur_speed)+',' + str(to) +',' + str(tmi)+','+ str(abcd[0].item(0))+','+ str(abcd[1].item(0))+','+ str(abcd[2].item(0))+','+ str(abcd[3].item(0)) + ';' #+','+str(battlev)
    return mergeData

def saveFile(data,mergeData): ##
...
def zumoThread(index, frameName, controlRate, mutex_tf, path):
    ...
    speed = 0.3*1.145 #linear scaling factor 1.145 to achieve 0.30 actual
    cur_speed = 0
    p_index = 0
    ...
    while isRun:
        ...
        try:
            ...
            #data = dataCollection(data,index,path,x,y,cur_speed) ##
            ...
            if res[0] == 0:
                ...
                if (controlInfo is not None) and (controlInfo != controlInfoPre):
                    if controlInfo[0] == 0:
                        ...
                        if mf.isFirst: 
                            isControlled = False
                            mergeData = mergeDataCollection(mergeData,index,x,y,cur_speed,time.time,mf.tmi,abcd)
                        else:
                            isControlled = True
                            ...
                            print "ABCD: "+str(abcd)
                            #mergeData = mergeDataCollection(mergeData,index,x,y,cur_speed,time.time()-mf.tinit,mf.tmi-mf.tinit,abcd) ##
...
def listener():
    ...
    t1 = Thread(target = zumoThread, args = (1, "zumoTest1", 0.01, mutex_tf, copy.deepcopy(Path.GetDefaultPath(0))))
    t1.start()
    t2 = Thread(target = zumoThread, args = (2, "zumoTest2", 0.01, mutex_tf, copy.deepcopy(Path.GetDefaultPath(0))))
    t2.start()
    t3 = Thread(target = zumoThread, args = (3, "zumoTest3", 0.01, mutex_tf, copy.deepcopy(Path.GetDefaultPath(0))))
    t3.start()
    ...
    #t12.start()
    #t13 = Thread(target = zumoThread, args = (13, "zumoTest13", 0.01, mutex_tf, copy.deepcopy(Path.GetDefaultPath(0))))
    #t13.start()
    #t14 = Thread(target = zumoThread, args = (14, "zumoTest14", 0.01, mutex_tf, copy.deepcopy(Path.GetDefaultPath(1))))
    #t14.start()
